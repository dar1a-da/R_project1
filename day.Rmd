...
sum(is.na(df))
```
Используем пакет **"visdat"** для визуализации типов переменных в наборе данных, расположения пропусков, отображения типов в колонках.
```{r}
library(ggplot2)
library(visdat)
...
NA присутствуют в 4-х переменных (temp, hum, windspeed, registered), в строках не пересекаются. 
...
Конвертируем из численных в факторные season, yr, mnth, holyday, weekday, workingday, weathersit
```{r}
...
## Гипотеза о поездках в будние и выходные дни
Рассмотрение вопроса об увеличенном количестве случайных пользователей в выходные дни и уменьшенном количестве зарегистрированных пользователей в выходные дни позволит скорректировать количество и расположение доступных велосипедов в зависимости от рабочего и выходного дня и обеспечить велосипедами всех желающих.

Н0 количество поездок зарегистрированных и случайных пользователей не зависит от того, какой день - рабочий или выходной.

Н1 количество поездок зарегистрированных и случайных пользователей зависит от того рабочий или выходной день, в выходные дни случайных пользователей будет больше, чем в рабочие дни, в будние дни количество зарегистрированных пользователей будет больше, чем в выходные дни.
...
По результатам теста Вилкоксона отвергаем нулевую гипотезу об одинаковом количестве поездок случайных пользователей в рабочий и выходной дни, так как очень низкое p-value, медиана в первой группе "Weekend" больше, чем в "Workday", значит в выходные дни количество поездок случайных пользователей больше.

```{r}
wilcox.test(registered ~ workingday, data = df_nona, correct = F, alternative = "less")
```
По результатам теста Вилкоксона отвергаем нулевую гипотезу об одинаковом количестве поездок зарегистрированных пользователей в рабочий и выходной дни, так как очень маленькое p-value. Подтверждается альтернативная гипотеза о том, что в будние дни количество зарегистрированных пользователей будет больше, чем в выходные дни, медиана в выходные дни меньше, чем в будние дни.

Построим график, отображающий количество поездок случайных и зарегистрированных пользователей в рабочие и нерабочие дни. Красными линиями показаны медианы. Для визуализации распределения числа поездок мы использовали **ggbeeswarm** (функцию geom_quasirandom), так как она позволяет избежать наложения точек и более наглядно отобразить индивидуальные наблюдения в дискретных данных, обеспечивает равномерное заполнение и чёткую картину плотности распределения.
```{r}
library(ggplot2)
library(ggbeeswarm)
library(tidyr)
